...
static inline QStringList getPluginPaths()
{
    ...
    pluginPath += QLatin1String(LISA_LIBRARY_BASENAME);
    pluginPath += QLatin1String("/lisa/plugins");
    ...
    pluginPath += QLatin1Char('/')
            + QLatin1String(Core::Constants::LISA_SETTINGSVARIANT_STR)
            + QLatin1Char('/');
#if !defined(Q_OS_MAC)
    pluginPath += QLatin1String("lisa");
#else
    pluginPath += QLatin1String("lisa");
#endif
    pluginPath += QLatin1String("/plugins/");
    pluginPath += QLatin1String(Core::Constants::LISA_VERSION_LONG);
    rc.push_back(pluginPath);
    return rc;
}
...
static inline QSettings *userSettings()
{
    ...
    const QString fromVariant = QLatin1String(Core::Constants::LISA_COPY_SETTINGS_FROM_VARIANT_STR);
...
int main(int argc, char **argv)
{
    ...
    if (settingsPath.isEmpty() && testOptionProvided) {
        settingsPath = QDir::tempPath() + QString::fromLatin1("/qtc-%1-test-settings")
                .arg(QLatin1String(Core::Constants::LISA_VERSION_LONG));
...
#endif
        if (translator.load(QLatin1String("lisa_") + locale, creatorTrPath)) {
