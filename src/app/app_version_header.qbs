    ...
    Transformer {
        ...
        prepare: {
            ...
            cmd.sourceCode = function() {
                ...
                content = content.replace(/(\n#define LISA_VERSION) .+\n/, "$1 " + project.lisa_version + "\n");
                content = content.replace(/(\n#define LISA_VERSION_MAJOR) .+\n/, "$1 " + project.lisa_version_major + "\n")
                content = content.replace(/(\n#define LISA_VERSION_MINOR) .+\n/, "$1 " + project.lisa_version_minor + "\n")
                content = content.replace(/(\n#define LISA_VERSION_RELEASE) .+\n/, "$1 " + project.lisa_version_release + "\n")
