...
exists($$depfile) {
    include($$depfile)
    isEmpty(LISA_PLUGIN_NAME): \
        error("$$basename(depfile) does not define LISA_PLUGIN_NAME.")
} else {
    isEmpty(LISA_PLUGIN_NAME): \
        error("LISA_PLUGIN_NAME is empty. Maybe you meant to create $$basename(depfile)?")
}
TARGET = $$LISA_PLUGIN_NAME

plugin_deps = $$LISA_PLUGIN_DEPENDS
plugin_test_deps = $$LISA_TEST_DEPENDS
plugin_recmds = $$LISA_PLUGIN_RECOMMENDS
...
defineReplace(dependencyName) {
    ...
    return($$LISA_PLUGIN_NAME)
}

# for substitution in the .json
dependencyList =
for(dep, plugin_deps) {
    dependencyList += "        { \"Name\" : \"$$dependencyName($$dep)\", \"Version\" : \"$$LISA_VERSION\" }"
}
for(dep, plugin_recmds) {
    dependencyList += "        { \"Name\" : \"$$dependencyName($$dep)\", \"Version\" : \"$$LISA_VERSION\", \"Type\" : \"optional\" }"
}
for(dep, plugin_test_deps) {
    dependencyList += "        { \"Name\" : \"$$dependencyName($$dep)\", \"Version\" : \"$$LISA_VERSION\", \"Type\" : \"test\" }"
}
...
isEmpty(PRECOMPILED_HEADER):PRECOMPILED_HEADER = $$PWD/shared/lisa_gui_pch.h

isEmpty(USE_USER_DESTDIR) {
    DESTDIR = $$LISA_PLUGIN_PATH
} else {
    win32 {
        DESTDIRAPPNAME = "lisa"
    ...
    DESTDIR = "$$DESTDIRBASE/$$DESTDIRAPPNAME/plugins/$$LISA_VERSION"
}
LIBS += -L$$DESTDIR
INCLUDEPATH += $$OUT_PWD
...
isEmpty(TARGET) {
    error("lisaplugin.pri: You must provide a TARGET")
}
...
!macx {
    target.path = $$LISA_PREFIX/$$LISA_LIBRARY_BASENAME/lisa/plugins
    INSTALLS += target
}
