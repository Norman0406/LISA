...
int C_FIR_filter::run (const std::complex<double> &in, std::complex<double> &out) {
	ibuffer[pointer] = in.real();
	qbuffer[pointer] = in.imag();
	counter++;
	if (counter == decimateratio)
        out = std::complex<double>(mac(&ibuffer[pointer - length], ifilter, length),
...
sfft::sfft(int len, int _first, int _last)
{
    ...
    delay  = new std::complex<double>[len];
	fftlen = len;
	first = _first;
	last = _last;
	ptr = 0;
	...
	k2 = 1.0;
	for (int i = 0; i < len; i++) {
        vrot_bins[i].vrot = std::complex<double>(K1 * cos (phi), K1 * sin(phi));
		phi += tau;
...
void sfft::run(const std::complex<double>& input, std::complex<double>* __restrict__ result, int stride )
{
    std::complex<double>& de = delay[ptr];
    const std::complex<double> z( input.real() - k2 * de.real(), input.imag() - k2 * de.imag());
	de = input;
