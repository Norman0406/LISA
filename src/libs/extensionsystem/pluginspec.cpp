...
/*!
    The plugin authors. This is valid after the PluginSpec::Read state is reached.
*/
QString PluginSpec::authors() const
{
    return d->authors;
}
...
bool PluginSpecPrivate::readMetaData(const QJsonObject &metaData)
{
    ...
    value = pluginInfo.value(QLatin1String(AUTHORS));
    if (!value.isUndefined() && !value.isString())
        return reportError(msgValueIsNotAString(AUTHORS));
